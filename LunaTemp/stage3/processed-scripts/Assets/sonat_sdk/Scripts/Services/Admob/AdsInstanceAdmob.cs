...
public partial class AdsInstanceAdmob : AdsInstance
{
    ...
    //public override void SetupAction(int index, IntAction onInitialized, BooleanAction onVideoAdsLoaded, BaseAction onVideoAdsRewarded,
    //    BooleanAction bannerShowed, bool bannerShow)
    //{
...
public partial class AdsInstanceAdmob : AdsInstance
{
    ...
    {
        UIDebugLog.Log0("AdmobId :" + nameof(BannerId) + ":" + GetLast(BannerId), LogType.Ads);
        UIDebugLog.Log0("AdmobId :" + nameof(InterstitialId) + ":" + GetLast(InterstitialId), LogType.Ads);
        UIDebugLog.Log0("AdmobId :" + nameof(VideoId) + ":" + GetLast(VideoId), LogType.Ads);
        UIDebugLog.Log0("AdmobId :" + nameof(OpenId) + ":" + GetLast(OpenId), LogType.Ads);
        UIDebugLog.Log0("AdmobId :" + nameof(NativeBannerId) + ":" + GetLast(NativeBannerId), LogType.Ads);
    ...
    public override void ShowDebugger()
    {
        ...
        {
            if (error != null)
                UIDebugLog.Log0("OpenAdInspector :" + error.GetMessage(), LogType.Ads);
        });
        UIDebugLog.Log0(GetType().Name + " " + nameof(ShowDebugger), LogType.Ads);
    }
...
//#if UNITY_IOS*
    ...
    public override bool IntersIsShowing()
    {
//        UIDebugLog.Log0(_intersState, LogType.Ads);
        return _intersState == AdsState.Showing;
    }

    public override bool VideoIsShowing()
    {
        //      UIDebugLog.Log0("_videoState" + _videoState, LogType.Ads);
...
//            Initialized = true;
//            CheckInitialized();
            UIDebugLog.Log0("admob : _initialized", LogType.Ads);
    ...
    public override void RequestNewAds()
    {
        UIDebugLog.Log0(nameof(RequestNewAds), LogType.Ads);
    ...
    private void RequestInterstitial()
    {
        if (string.IsNullOrEmpty(InterstitialId))
        {
            ...
            UIDebugLog.Log0("RequestInterstitial Fail InterstitialId=null", LogType.Ads);
            return;
        }
        ...
        if (logFail) UIDebugLog.Log0("AdsInstanceAdmob: RequestInterstitial" + GetLast(InterstitialId), LogType.Ads);
    ...
    private void RequestOpenAds()
    {
        if (string.IsNullOrEmpty(OpenId))
        {
            ...
            UIDebugLog.Log0("RequestOpenAds Fail OpenId null", LogType.Ads);
            return;
        }
        ...
        if (string.IsNullOrEmpty(OpenId))
        {
            UIDebugLog.Log0("AdsInstanceAdmob RequestOpenAds Err : OpenAdId null", LogType.Ads);
            return;
        }

        if (logFail) UIDebugLog.Log0("AdsInstanceAdmob: RequestOpenAds", LogType.Ads);
        ...
        AppOpenAd.Load(OpenId, ScreenOrientation.Portrait, adRequest,
            (ad, error) =>
            {
                // if error is not null, the load request failed.
                if (error != null || ad == null)
    ...
    public void RequestRewardedAd()
    {
        if (string.IsNullOrEmpty(VideoId))
        {
            ...
            UIDebugLog.Log0("RequestRewardedAd Fail VideoId null", LogType.Ads);
            return;
        }
        ...
        if (logFail) UIDebugLog.Log0("AdsInstanceAdmob: CreateAndLoadRewardedAd", LogType.Ads);
        ...
        AdsPlatform platform)
    {
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            if (logFail) UIDebugLog.Log0(adType + " " + nameof(HandleAdPaidEvent) + " event received ", LogType.Ads);
            SonatAnalyticTracker.LogRevenue(platform, adapter, adValue.Value / 1000000f,
...
//            }
    ...
    private void HandleOnRewardAdFailedToShow(AdError addError)
    {
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            if (logFail)
                UIDebugLog.Log0(nameof(HandleOnRewardAdFailedToShow) + " retryDelay-" + retryDelay, LogType.Ads);
        });
    }
...
//            _timeShowRewardsAds = -1000;
    ...
    private void HandleUserEarnedReward(Reward reward)
    {
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            if (logFail) UIDebugLog.Log0(nameof(HandleUserEarnedReward), LogType.Ads);
            VideoRewarded.Action.Invoke();
        });
    }
    ...
    public override void ShowInterstitial(bool isOnFocus)
    {
        UIDebugLog.Log0("useOpenAdsAsInterstitial.Value " + useOpenAdsAsInterstitial.Value, LogType.Ads);
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            if (logFail && loadAdError != null)
            {
                ...
                UIDebugLog.Log0(
                    ...
                    LogType.Ads);
#endif
                UIDebugLog.Log0($"Reload Interstitial in {(float) retryDelay} seconds", LogType.Ads);
            }
        });
    ...
    private void HandleOnInterstitialAdOpening()
    {
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            if (logFail) UIDebugLog.Log0(nameof(HandleOnInterstitialAdOpening) + " event received", LogType.Ads);
        });
    }
...
//        if (args != null)
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            //            Time.timeScale = 1f;
            if (logFail) UIDebugLog.Log0(nameof(HandleOnInterstitialAdClosed) + " event received", LogType.Ads);
            _interstitial?.Destroy();
            StartCoroutine(IeRequestInters(.5f));
    ...
    private void HandleOnOpenAdsOpened()
    {
        ...
        {
            AdsManager.OpenAdsOrInterstitialOpened = true;
            if (logFail) UIDebugLog.Log0(nameof(HandleOnOpenAdsOpened) + " event received", LogType.Ads);
        });
    }
    ...
    private void HandleOnOpenAdsClosed()
    {
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            //        if (Kernel.IsReady() && !Kernel.Pause)
            //            Time.timeScale = 1f;
            if (logFail) UIDebugLog.Log0(nameof(HandleOnOpenAdsClosed) + " event received", LogType.Ads);
            _appOpenAds?.Destroy();
            StartCoroutine(IeRequestOpenAds(.5f));
    ...
    private void HandleOnOpenAdLoaded()
    {
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            if (logFail)
                UIDebugLog.Log0(nameof(HandleOnOpenAdLoaded) + " event received: " + GetLast(OpenId), LogType.Ads);
            //            
    ...
    public void HandleOnOpenAdLeavingApplication(object sender, EventArgs args)
    {
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            if (logFail)
                UIDebugLog.Log0(nameof(HandleOnOpenAdLeavingApplication) + " event received", LogType.Ads);
        });
    }
