...
public class AdsManager : BaseService
{
    ...
    public override bool Initialized
    {
        ...
        get { return _initilized && (!CalculateWaitConsent || ConsentReady); }
...
//            onAdsInitialized.Destroy();
    ...
    private bool CheckConditionTimeGap(int level)
    {
        ...
        return Time.realtimeSinceStartup - _lastTimeShowInters >= timeGap;
    }
    ...
    public AdsInstance IsInterstitialAdsValid(int level, bool conditionLevel)
    {
        ...
        if (IsNoAds())
        {
            UIDebugLog.Log("Couldn't start inters Ads : NoAds=true", LogType.Ads);
            return null;
        }
...
//        }
        ...
        if (BannerOpened)
        {
            UIDebugLog.Log($"Couldn't start inters Ads :'{nameof(BannerOpened)}:{BannerOpened}", LogType.Ads);
            return null;
        }
        ...
        {
            UIDebugLog.Log(
                $"Couldn't start inters Ads :'{nameof(OpenAdsOrInterstitialOpened)}:{OpenAdsOrInterstitialOpened}",
                LogType.Ads);
            return null;
        }
        ...
        if (VideoOpened)
        {
            UIDebugLog.Log($"Couldn't start inters Ads :'{nameof(VideoOpened)}:{VideoOpened}", LogType.Ads);
            return null;
        }

        if (AppLeaving > 0)
        {
            UIDebugLog.Log($"Couldn't start inters Ads :'{nameof(AppLeaving)}:{AppLeaving}", LogType.Ads);
            return null;
        }
        ...
        if (conditionLevel)
        {
            ...
            {
                UIDebugLog.Log(
                    $"Couldn't start inters Ads : Level condition not valid {nameof(level)}={level} < {condition}(condition)",
                    LogType.Ads);
                return null;
            }
        }
        ...
        {
            UIDebugLog.Log(
                $"Couldn't start inters Ads : Time gap invalid {Time.realtimeSinceStartup - _lastTimeShowInters} < interstitial_time_gap={(RemoteConfigKey.interstitial_time_gap).GetValueInt()})",
                LogType.Ads);
            return null;
        }
        ...
        {
            UIDebugLog.Log($"Couldn't start inters Ads : No ads ready, total source ={_adsInstances.Length}", LogType.Ads);
...
//        Time.timeScale = startTimeScale;
    ...
    private void ShowInters(SonatLogShowInterstitial log,bool isOnFocus,AdsInstance interstitialAdsSource)
    {
        ...
        _lastTimeShowInters = Time.realtimeSinceStartup;
        ...
        UIDebugLog.Log("ShowInterstitial invoked", LogType.Ads);
    }
    ...
    protected virtual void OnApplicationFocus(bool hasFocus)
    {
        ...
        {
            UIDebugLog.Log("RemoteConfigKey.turn_off_focus_ads.GetValueBoolean()"+RemoteConfigKey.turn_off_focus_ads.GetValueBoolean(), LogType.Ads);
            return;
        }
        ...
        if (FireBaseController.FireBaseRemoteReady)
        {
            ...
            if (diff.Seconds > RemoteConfigKey.seconds_to_dispose_ads.GetValueInt())
            {
                ...
                UIDebugLog.Log("Show inters fail :Dispose all ads", LogType.Ads);
                return;
            }
        }
        ...
        if (!Kernel.IsReady())
        {
            UIDebugLog.Log("Show inters fail : Kernel not ready", LogType.Ads);
            return;
        }

        if (Time.time < 3)
        {
            UIDebugLog.Log("Show inters fail : Time too soon < 3s", LogType.Ads);
            return;
        }
...
//            Kernel.Resolve<FireBaseController>().LogEvent("AdsManagerOnFocus");
        ...
        {
            UIDebugLog.Log("Show inters fail : OnApplicationFocusValid is False", LogType.Ads);
            return;
        }
        ...
        {
            UIDebugLog.Log("Show inters fail : _applicationOutTime too long", LogType.Ads);
            _applicationOutTime = Time.realtimeSinceStartup;
            return;
        }
        ...
        if (TestingSettings.TurnOffOnFocusAds)
            return;
        UIDebugLog.Log("Ready to check show inters", LogType.Ads);
        ShowIntersOnFocus();
#else
...
//    
    ...
    private void CheckLoadLoadingTransition()
    {
        if (LoadingTransition.Instance == null)
        {
            var transition = FindObjectOfType<LoadingTransition>();
            if (transition != null)
