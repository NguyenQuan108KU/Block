...
public class BB10_PattemCreater : MonoBehaviour
{
    ...
    public List<BB10_BrickCubeUnit> CreatePattem(Types type, [Bridge.Ref] Vector2 pos, float scale)
    {
        return Create(type, pos, scale);
    }

    List<BB10_BrickCubeUnit> Create(Types thisType, [Bridge.Ref] Vector2 pos, float scale)
    {
        if (!dataInfor.ContainsKey(thisType))
        {
            Debug.LogError("Pattern type not found: " + thisType);
            return new List<BB10_BrickCubeUnit>();
        }
    ...
    public BB10_BrickCubeUnit CreateABlock([Bridge.Ref] Vector2 pos, float scale)
    {
