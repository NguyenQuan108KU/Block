...
public class AskGDPRScript : AskScriptBase
{
    ...
    private bool LoadGdpr()
    {
        ...
        if (PlayerPrefs.GetInt(PlayerPrefEnum.cache_consent_admob.ToString()) == 1)
        {
            UIDebugLog.Log0($" Consent cache_consent_admob cached",LogType.Service,true);
            ConsentReady = true;
            if (showAttAfter)
                AdsManager.AskAtt();
            return ConsentReady;
        ...
        UIDebugLog.Log($" ConsentInformation.Update",LogType.Service,true);
    ...
    {
        UIDebugLog.Log($"duong OnConsentInfoUpdated IsConsentFormAvailable:{ConsentInformation.IsConsentFormAvailable()} error{error}",LogType.Service,true);
        ...
        {
            // Handle the error.
            UIDebugLog.Log($"duong OnConsentInfoUpdated FormError:{error.Message}",LogType.Service,true);
            return;
        }

        if (ConsentInformation.IsConsentFormAvailable())
        {
            UIDebugLog.Log($"duong OnConsentInfoUpdated IsConsentFormAvailable:{ConsentInformation.IsConsentFormAvailable()}",LogType.Service,true);
    ...
    {
        UIDebugLog.Log0($"duong OnLoadConsentForm ConsentInformation.ConsentStatus{ConsentInformation.ConsentStatus}",LogType.Service, true);
        if (
