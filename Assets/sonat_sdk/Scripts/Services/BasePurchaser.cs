...
public abstract class BasePurchaser : BaseService
{
    ...
    public void Buy(int shopItemKey, Action action)
    {
...
public abstract class BasePurchaser : BaseService, IStoreListener,IDetailedStoreListener
{
    ...
    private KeyValuePair<int, Action> _onSuccess;
    public ShopItemKeyAction onInAppPurchased = new ShopItemKeyAction();
    ...
    public static float sn_ltv_iap
    {
        ...
        set
        {
            ...
            {
                Kernel.Resolve<FireBaseController>().SetUserProperty(nameof(sn_ltv_iap),value.ToString(CultureInfo.InvariantCulture));
                PlayerPrefs.SetFloat(nameof(sn_ltv_iap),value);
            }
        }
    }
...
//#endif
    ...
    private IEnumerator WaitForApplicationFocus()
    {
        ...
        {
            StartCoroutine(LoadingTransition.Instance.LoadingIn(0));
            show = true;
        }
        float t = 0f;
    ...
    public void Buy(int id, Action onSuccess)
    {
        #if !UNITY_EDITOR
        ...
        _onSuccess = new KeyValuePair<int, Action>(id, onSuccess);
        var item = StoreProductDescriptors.Find(x => x.key == id);
        if (_dictionary.ContainsKey(item.StoreProductId))
            _dictionary[item.StoreProductId] = onSuccess;
        else
            _dictionary.Add(item.StoreProductId,onSuccess);
        _isBuy = true;
    ...
    private readonly Dictionary<string,Action> _dictionary = new Dictionary<string, Action>();
...
//        }).Post();
    ...
    private IEnumerator IeVerify(WWWForm form, Action afterVerify)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post(verifyUrl, form))
        {
            ...
            UIDebugLog.Log(www.responseCode);
    ...
    public bool IsSubcribed(string storeProductId, out int expiredIn)
    {
        ...
        try
        {
            ...
            if (isSubscribed)
                expiredIn = SdkExtenstions.GetEpochDate(subscriptionInfo.getExpireDate()) - SdkExtenstions.GetEpochDate();
            return isSubscribed;
        }
