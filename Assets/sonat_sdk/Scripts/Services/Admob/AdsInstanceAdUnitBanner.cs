...
public partial class AdsInstanceAdmob
{
    ...
    {
        UIDebugLog.Log0("IeRequestBanner", LogType.Ads);
    ...
    private void RequestBanner()
    {
        if (string.IsNullOrEmpty(BannerId))
        {
            ...
            UIDebugLog.Log0("RequestBanner Fail BannerId null", LogType.Ads);
            return;
        }
        ...
        {
            _bannerState = AdsState.Failed;
            UIDebugLog.Log0("RequestBanner Fail RemoteConfigKey.no_banner", LogType.Ads);
            return;
        }
        ...
        {
            UIDebugLog.Log0("Banner not show" + nameof(TestingSettings.TurnOffBanner), LogType.Ads);
            return;
        }
#endif
        ...
        if (AdsManager.NoAds)
        {
            UIDebugLog.Log0("Banner not show " + nameof(AdsManager.NoAds), LogType.Ads);
            return;
        }
        ...
        {
            UIDebugLog.Log0("Banner not show " + nameof(AdsManager.NotShowBanner), LogType.Ads);
            return;
        }
        ...
        if (Initialized)
        {
            if (logFail) UIDebugLog.Log0("AdsInstanceAdmob: RequestBanner" + GetLast(BannerId), LogType.Ads);
            _bannerView?.Destroy();
            _bannerView = null;
    ...
    {
        UIDebugLog.Log(_bannerState, LogType.Ads);
    ...
    private void HandleBannerAdLoaded()
    {
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            if (!string.Equals(_lastBanner, BannerId, StringComparison.Ordinal))
            {
                ...
                UIDebugLog.Log0("HandleBannerAdLoaded=" + GetLast(_lastBanner), LogType.Ads);
            }
    ...
    private void HandleBannerAdFailedToLoad(AdError adError)
    {
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            if (logFail)
            {
                ...
                UIDebugLog.Log0(
                    $"{nameof(HandleBannerAdFailedToLoad)} ({GetLast(BannerId)})  event received with message: {mess}",
                    LogType.Ads);
#endif
                UIDebugLog.Log0(
                    $"Reload Banner in {(float) retryDelay} seconds",
                    LogType.Ads);
            }
    ...
    private void HandleBannerAdOpened()
    {
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            StartCoroutine(InvokeBanner(false));
            if (logFail) UIDebugLog.Log0(nameof(HandleBannerAdOpened) + " event received", LogType.Ads);
        });
    }
    ...
    private void HandleBannerAdClosed()
    {
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            StartCoroutine(InvokeBanner(false));
            if (logFail) UIDebugLog.Log0(nameof(HandleBannerAdClosed) + " event received", LogType.Ads);
