...
public partial class AdsInstanceAdmob
{
    ...
    {
        UIDebugLog.Log0("IeRequestNativeBanner", LogType.Ads);
    ...
    private void RequestNativeBanner()
    {
        if (string.IsNullOrEmpty(NativeBannerId))
        {
            ...
            UIDebugLog.Log0("RequestNativeBanner Fail NativeBannerId null",LogType.Ads);
            return;
        }
        ...
        {
            UIDebugLog.Log0("NativeBanner not show" + nameof(TestingSettings.TurnOffBanner), LogType.Ads);
            return;
        }
#endif
        if(!LoadNative)
        ...
        {
            UIDebugLog.Log0("NativeBanner not show" + nameof(AdsManager.NotShowBanner), LogType.Ads);
            return;
        }
        ...
        if (Initialized)
        {
            if (logFail) UIDebugLog.Log0("AdsInstanceAdmob: RequestNativeBanner"+GetLast(NativeBannerId), LogType.Ads);
    ...
    private void HandleNativeBannerAdOpened()
    {
        ...
        {
            StartCoroutine(InvokeNativeBanner(false));
            if (logFail) UIDebugLog.Log0(nameof(HandleNativeBannerAdOpened) + " event received", LogType.Ads);
        });
    }
    ...
    private void HandleNativeBannerAdClosed()
    {
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            StartCoroutine(InvokeNativeBanner(false));
            if (logFail) UIDebugLog.Log0(nameof(HandleNativeBannerAdClosed) + " event received", LogType.Ads);
